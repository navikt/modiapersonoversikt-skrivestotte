...
export type FieldState = [string, React.ChangeEventHandler, Dispatch<SetStateAction<string>>];

export function useFieldState(initialState: string | (() => string)): FieldState {
...
export function useForceAllwaysInViewport(selector: string, deps: DependencyList = []) {
...
export function useFetch<TYPE>(url: RequestInfo, option?: RequestInit): UseFetchHook<TYPE> {
